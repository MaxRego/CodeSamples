...
>
  ...
  <xsl:variable name="max_seq_len">
    <xsl:for-each select="document('job1_cisml.xml')/cis:cis-element-search/cis:multi-pattern-scan/mem:match" >
  ...
  <xsl:variable name="seq_is_dna">
    <xsl:choose>
      <xsl:when test="/job1_motifsXML/alphabet/text() = 'nucleotide'">
  ...
  <xsl:template name="html-head">
    <head>
      ...
      <script type="text/javascript">
        var motifs = new Array();<xsl:text></xsl:text>
        <xsl:for-each select="/job1_motifsXML/motif">
          <xsl:text>        motifs[&quot;</xsl:text><xsl:value-of select='@name'/><xsl:text>&quot;] = new Motif(&quot;</xsl:text>
          <xsl:value-of select='@name'/><xsl:text>&quot;, &quot;</xsl:text>
          <xsl:value-of select='/job1_motifsXML/alphabet/text()'/><xsl:text>&quot;, &quot;</xsl:text>
          <xsl:value-of select='@best-possible-match'/><xsl:text>&quot;, &quot;</xsl:text>
          <xsl:value-of select='@best-possible-rc-match'/><xsl:text>&quot;)</xsl:text>
        </xsl:for-each>
        ...
        function Sequence_append_labels(container, start, width, is_rc) {
          ...
          while (iter.has_next()) {
            ...
            var top_label = "";
            <xsl:variable name="db_dna" select="/job1_motifsXML/alphabet/text() = 'nucleotide'"/>
...
<!--
  <xsl:template name="top">
    <a name="top"/>
    <div class="pad1">
      <h1><img src="http://meme.nbcr.net/meme/doc/images/job1_motifsXML-logo.png" 
...
-->
  ...
  <xsl:template name="inputs" >
    ...
    <div class="box">
      ...
      <div class="pad">
        <p>
          <xsl:text>The sequence</xsl:text>
          &quot;<xsl:value-of
            select="document('job1_cisml.xml')/cis:cis-element-search/cis:parameters/cis:sequence-file/text()"
          />&quot;
          <xsl:text>was supplied.</xsl:text>
        </p>
        <table class="padded-table" border="0" >
          ...
          <tbody>
              <tr>
                <td class="tnum"><xsl:value-of select="/job1_motifsXML/sequence-data/@num-sequences"/></td>
                <td class="tnum"><xsl:value-of select="/job1_motifsXML/sequence-data/@num-residues"/></td>
              </tr>
          </tbody>
          <tfoot>
          </tfoot>
      ...
      <h4>Motifs</h4>
      <div class="pad">
        <p>The following motifs were supplied
        <!-- &quot;<xsl:value-of
          select="document('job1_cisml.xml')/cis:cis-element-search/cis:parameters/cis:pattern-file/text()"
        />&quot; -->
        </p>
        <xsl:variable name="stranded" select="string-length(/job1_motifsXML/motif[1]/@best-possible-rc-match)"/>
        <table class="padded-table" border="0" >
          <thead>
            <tr>
              <th>&nbsp;</th>
    ...
    <div class="box">
      ...
      <div class="pad">
        <p>
          ...
          <xsl:value-of 
           select="count(document('job1_cisml.xml')/cis:cis-element-search/cis:multi-pattern-scan)"/> matches 
          has a <i>q</i>-value less than 
          <xsl:value-of
          select="document('job1_cisml.xml')/cis:cis-element-search/cis:parameters/cis:site-pvalue-cutoff/text()"/>.
          <br />
          The motif matches shown have a position p-value less than 
          <xsl:value-of
          select="document('job1_cisml.xml')/cis:cis-element-search/cis:parameters/cis:site-pvalue-cutoff/text()"/>.<br />
          -->
          
          <b>Click on the arrow</b> (&more;) next to the block diagram to view more information about a sequence.
        </p>
        <xsl:call-template name="job1_motifsXML_legend"/>
        <table id="tbl_sequences" style="width:100%; table-layout:fixed;" border="0">
          ...
          <tbody>
            <xsl:for-each select="document('job1_cisml.xml')/cis:cis-element-search/cis:multi-pattern-scan">
  ...
  <xsl:template name="motif_index">
    ...
    <xsl:for-each select="document('job1_motifsXML.xml')/job1_motifsXML/motif">
  ...
  <xsl:template name="job1_motifsXML_legend">
  ...
  <xsl:template name="data" >
    <xsl:text></xsl:text>
    <xsl:comment >This data is used by the javascript to create the detailed views</xsl:comment><xsl:text></xsl:text>
    <form>
      <xsl:variable name="type">
        <xsl:value-of select="/job1_motifsXML/alphabet/text()"/>
      </xsl:variable>
      <xsl:for-each select="document('job1_cisml.xml')/cis:cis-element-search/cis:multi-pattern-scan/mem:match">
        ...
        <xsl:variable name="hits">
          ...
          <xsl:for-each select="../cis:pattern/cis:scanned-sequence/cis:matched-element"> 
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="@stop" /><xsl:text></xsl:text> 
              </xsl:otherwise>
            </xsl:choose>
            <xsl:value-of select="../../@name" /><xsl:text></xsl:text>
            <xsl:choose>
            ...
            <xsl:value-of select="./cis:sequence/text()"/><xsl:text></xsl:text>
          </xsl:for-each>
        </xsl:variable>
        <input type="hidden" id="{@cluster-id}_len" value="{@stop - @start + 1}" /><xsl:text></xsl:text>
        <input type="hidden" id="{@cluster-id}_combined_pvalue" value="{@pvalue}" /><xsl:text></xsl:text>
        <input type="hidden" id="{@cluster-id}_type" value="{$type}" /><xsl:text></xsl:text>
        <input type="hidden" id="{@cluster-id}_segs" value="{$segs}" /><xsl:text></xsl:text>
        <input type="hidden" id="{@cluster-id}_hits" value="{$hits}" /><xsl:text></xsl:text>
      </xsl:for-each>
    </form>
  </xsl:template>
  ...
  <xsl:template name="pick-colour">
    <xsl:param name="sym"/>
    <xsl:param name="isnuc" select="/job1_motifsXML/alphabet/text()= 'nucleotide'" />
    <xsl:choose>
      <xsl:when test="$isnuc">
        <xsl:choose>
          <xsl:when test="$sym = 'A'">
